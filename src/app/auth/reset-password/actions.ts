...
export default async function ResetPasswordAction(state: any, formdata: FormData) {
    try {
        ...
        if (!authSession || !userEmail) {
            return {
                type: "ERROR", msg: "Session Expired"
            }
        }

        const validate = ValidateResetPassword.safeParse({ newPassword, copyPassword })
