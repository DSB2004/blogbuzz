...
const addSaveQuery = "INSERT INTO SAVES (BLOG_ID,SAVED_BY) VALUES (?,?);";
...
export async function PUT(request: NextRequest) {
  ...
  try {
    ...
    try {
      await executeQuery(connection, addSaveQuery, [blog_id, user]);
    } catch (err) {
      return NextResponse.json({ msg: "Already saved" }, { status: 400 });
    }

    return NextResponse.json({ msg: "Save Added" });
  } catch (err) {
